...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://13.63.138.239:3000"],
...
def triage(req: QueryRequest):
    try:
        ...
        # Extract outputs
        try:
            validator_output = raw_result.tasks_output[3].raw
            response_output = raw_result.tasks_output[4].raw
        except Exception:
            validator_output = ""
            response_output = ""

        parsed = extract_json(validator_output)
...
def list_tickets(
    ...
    for t in cursor:
        ...
        tickets.append({
            ...
            "urgency": parsed.get("urgency"),          # ✅ NEW
            "risk_level": parsed.get("risk_level"),    # ✅ optional but useful
